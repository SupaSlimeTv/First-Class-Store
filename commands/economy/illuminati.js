...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('illuminati')
    .setDescription('🔺 The Illuminati — join, expose, or sell your soul.')
    .addSubcommand(s => s.setName('found').setDescription('Found the Illuminati in this server (costs $250,000)'))
    .addSubcommand(s => s.setName('status').setDescription('View the Illuminati and your standing'))
  ...
  async execute(interaction) {
    ...
    if (sub === 'found') {
      ...
      // Create the Illuminati Discord role and assign to founder
      let roleNote = '';
      try {
        const role = await interaction.guild.roles.create({
          name: '🔺 Illuminati', color: 0x1a1a2e, hoist: true, mentionable: false,
          reason: 'Illuminati membership tracking',
        });
        org.memberRoleId = role.id;
        const guildMember = await interaction.guild.members.fetch(userId).catch(() => null);
        if (guildMember) await guildMember.roles.add(role).catch(() => null);
        roleNote = `\n\n**⚙️ Server admin setup required:**\nGo to **Server Settings → Integrations → ${interaction.client.user.username} → /illuminatiops** and grant the \`🔺 Illuminati\` role access so members can see member-only commands.`;
      } catch {}
      ...
      return interaction.reply({ embeds:[new EmbedBuilder()
        ...
        .setDescription(`<@${userId}> has established the shadow order.\n\nThe **${fmtMoney(INITIATION_FEE)}** initiation fee has been deposited into the vault.\n\nYou are the **⚡ Grandmaster**. Choose your Elders wisely and establish factions to expand your power.${roleNote}`)
        .setFooter({ text:'Member ops: /illuminatiops · Membership is invite-only · Max 13 members' })
      ]});
    }
    ...
    if (sub === 'excommunicate') {
      ...
      // Remove Illuminati role
      if (org.memberRoleId) {
        const guildMember = await interaction.guild.members.fetch(target.id).catch(() => null);
        if (guildMember) await guildMember.roles.remove(org.memberRoleId).catch(() => null);
      }
