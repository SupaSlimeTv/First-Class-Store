...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('myheat')
    .setDescription('Check your current police heat level and record.'),
  ...
  async execute(interaction) {
    ...
    await interaction.reply({ embeds: [new EmbedBuilder()
      ...
      .setTitle(`🌡️ Police Heat — ${level.name}`)
      .setDescription(`\`[${bar}] ${heat}/100\``)
      .addFields(
        { name: '🌡️ Heat',       value: `${heat}/100`,                               inline: true },
        { name: '🚔 Arrests',    value: (record.arrests || 0).toString(),             inline: true },
        { name: '⏰ Jail Status', value: jailed ? `🔒 ${jailLeft} min left` : '🟢 Free', inline: true },
