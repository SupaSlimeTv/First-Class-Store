...
module.exports = {
  ...
  async execute(interaction) {
    ...
    const speedBuff    = getConsumeBuff(interaction.user.id, 'speed');
    const effectiveCooldown = Math.floor(COOLDOWN_MS * (1 - speedBuff / 100));

    if (user.lastWork && now - user.lastWork < effectiveCooldown) {
      const left = effectiveCooldown - (now - user.lastWork);
    ...
    // Apply work buffs
    const workBoost = getConsumeBuff(interaction.user.id, 'work_boost');
    const focused   = getConsumeBuff(interaction.user.id, 'focused');
    if (workBoost) earned = Math.floor(earned * (1 + workBoost / 100));
    if (focused)   earned = Math.floor(earned * 2);
    user.wallet  += earned;
    user.lastWork = now;
