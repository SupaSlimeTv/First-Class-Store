...
module.exports = {
  ...
  async execute(interaction) {
    ...
    // Apply lucky buff to payout
    const { getConsumeBuff } = require('../../utils/consumeBuffs');
    const lucky = getConsumeBuff(interaction.user.id, 'lucky');
    if (lucky && result.amount > 0) result.amount = Math.floor(result.amount * (1 + lucky / 100));
